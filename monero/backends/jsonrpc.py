...
class JSONRPCDaemon(object):
    ...
    def block(self, bhash=None, height=None):
        ...
        data['count'] = 1
        res2 = self.raw_jsonrpc_request('get_coinbase_tx_sum', data)
        if res['status'] == 'OK':
            bhdr = res['block_header']
            sub_json = json.loads(res['json'])
            data = {
                'size': bhdr['block_size'],
                'height': bhdr['height'],
                'timestamp': bhdr['timestamp'],
                'num_txes': bhdr['num_txes'],
                'version': (bhdr['major_version'], bhdr['minor_version']),
                'difficulty': bhdr['difficulty'],
                'nonce': bhdr['nonce'],
                'reward': from_atomic(res2['emission_amount']),
                'fee': from_atomic(res2['fee_amount']),
            }
            return Block(**data)
        raise exceptions.BackendException(res['status'])
